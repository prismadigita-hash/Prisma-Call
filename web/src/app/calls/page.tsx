...
const selectCls =
  'rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100'
        ...
        <Card className="mb-5">
          <CardBody>
            <form className="flex flex-wrap items-end gap-3">
              ...
              <Link href="/calls" className="px-2 py-2 text-sm text-slate-500 hover:text-slate-800">Limpar</Link>
            </form>
          </CardBody>
        </Card>
        ...
        {calls.length === 0 ? (
          ...
          <Card>
            <div className="divide-y divide-border">
              {calls.map((call) => (
                <Link key={call.id} href={`/calls/${call.id}`} className="flex items-center gap-4 px-5 py-3.5 hover:bg-slate-50">
                  ...
                  <span className="w-12 text-right text-base font-bold text-slate-700">{fmtScore(call.overall_score)}</span>
                </Link>
              ))}
            </div>
          </Card>
