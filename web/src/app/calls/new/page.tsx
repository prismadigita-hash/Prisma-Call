        ...
        <Card className="max-w-2xl">
          <CardBody>
            <form action={createCall} className="space-y-4">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-300">Link da gravação (opcional)</label>
                <input name="recording_url" type="url" className={inputCls} placeholder="https://…" />
              </div>

              <div>
                <label className="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-300">Transcrição</label>

                {/* Upload de arquivo */}
                <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 dark:bg-slate-800/60 px-3 py-3">
                  <p className="mb-2 flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                    <Upload size={16} className="text-slate-400" />
                    <span><span className="font-medium text-slate-700 dark:text-slate-300">Anexar arquivo</span> — .txt, .md, .vtt, .srt (máx. 2 MB)</span>
                  </p>
                  <input
                    type="file"
                    name="transcript_file"
                    accept=".txt,.md,.vtt,.srt,.text,text/plain"
                    className="block w-full text-sm text-slate-600 dark:text-slate-300 file:mr-3 file:rounded-lg file:border-0 file:bg-indigo-600 file:px-3 file:py-1.5 file:text-sm file:font-semibold file:text-white hover:file:bg-indigo-700"
                  />
                </div>
